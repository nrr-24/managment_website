...
export default function EditModifierPage() {
    ...
    const currentData = JSON.stringify({ name, nameAr, isRequired, maxSelection, items });
        ...
        getModifierGroup(rid, mid).then(m => {
            if (m) {
                ...
                initialDataRef.current = JSON.stringify({ 
                    ...
                    items: m.items?.map(i => ({ id: i.id, name: i.name || "", nameAr: i.nameAr || "", price: i.price?.toString() || "" })) || []
                });
            }
            setLoaded(true);
        });
